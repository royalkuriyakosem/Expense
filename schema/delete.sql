...
-- Trigger to update total_expense after an expense is deleted
...
BEGIN
    ...
    -- Check if the category exists in expense_each_category
    SELECT total_expense INTO existing_total
    FROM expense_each_category
    WHERE category = OLD.category;

    IF existing_total IS NOT NULL THEN
        -- If it exists, subtract the deleted amount
        UPDATE expense_each_category
...
-- Trigger to update total_income after an income is deleted
CREATE TRIGGER incomes_AFTER_DELETE 
AFTER DELETE ON incomes 
FOR EACH ROW 
BEGIN
    DECLARE existing_income_total DECIMAL(10, 2);

    -- Check if the source exists in income_per_source
    SELECT total_income INTO existing_income_total
    FROM income_per_source
